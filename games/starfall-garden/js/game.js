...
SG.Game = class {
  ...
  _onDash() {
    if (this.state === SG.STATE.PLAYING) {
      if (this.player.tryDash()) {
        this.audio.dash();
        this.particles.burst(this.player.x, this.player.y, 8, { color: SG.COLORS.healthyCyan, life: 0.35, size: 3, speedMin: 40, speedMax: 140, glow: true });
      }
    }
  }

  // Computes what pressing "interact" would currently do, for HUD/mobile-label/world-highlight readability.
  _computeActionHint() {
    const patch = this.world.patchAtLocal(this.player.x, this.player.y);
    if (!patch) return null;
    if (patch.state === SG.TERRAIN.DEAD) {
      if (!this.world.isUnlocked(patch)) return { type: 'locked', patch, label: 'OUTER RING LOCKED' };
      const cost = SG.Plants.restoreCost(this.world, this.player.x, this.player.y, SG.CONFIG.restoreCost, this.player);
      return { type: 'restore', patch, label: 'RESTORE', cost, afford: this.player.fragments >= cost };
    }
    if (SG.Plants.canPlant(patch)) {
      const cfg = SG.PLANT_TYPES[this.selectedPlant];
      const cost = Math.max(1, Math.round(cfg.cost * this.player.plantCostMul));
      return { type: 'plant', patch, label: 'PLANT ' + cfg.name.toUpperCase(), cost, afford: this.player.fragments >= cost };
    }
    if (patch.plant) return { type: 'occupied', patch, label: 'PATCH OCCUPIED' };
    return null;
  }

  _showFeedback(text) { this._actionFeedback = { text, timer: 1.8 }; }
  ...
  _onInteract() {
    ...
    if (hint.type === 'locked') {
      this._showFeedback('OUTER RING LOCKED');
      this.audio.denied();
    } else if (hint.type === 'occupied') {
      this._showFeedback('PATCH OCCUPIED');
      this.audio.denied();
    } else if (hint.type === 'restore') {
      if (!hint.afford) {
        this._showFeedback(`NEED ${hint.cost - Math.floor(this.player.fragments)} MORE FRAGMENTS`);
        this.audio.denied();
        return;
      }
      this.player.fragments -= hint.cost;
      this.world.restorePatch(hint.patch, hint.patch.biome === 'crystalgrove' && Math.random() < 0.35);
      this.audio.restore();
      this.particles.burst(hint.patch.x, hint.patch.y, 16, { color: SG.COLORS.healthyCyan, life: 0.6, size: 4, speedMin: 20, speedMax: 90, glow: true });
    } else if (hint.type === 'plant') {
      if (!hint.afford) {
        this._showFeedback(`NEED ${hint.cost - Math.floor(this.player.fragments)} MORE FRAGMENTS`);
        this.audio.denied();
        return;
      }
      const result = SG.Plants.tryPlant(this.world, hint.patch, this.selectedPlant, this.player);
      if (result.ok) {
        this.runStats.plantsGrown++;
        this.audio.plant();
        this.particles.burst(hint.patch.x, hint.patch.y, 10, { color: SG.PLANT_TYPES[this.selectedPlant].color, life: 0.5, size: 3, speedMin: 15, speedMax: 60, glow: true });
  ...
  _startRun(hardMode, endless) {
    ...
    this._lastUnlockedRing = 0;
    this._ringUnlockBanner = null;
    this._ringPulse = 0;
  ...
  update(dt) {
    ...
    this.actionHint = this._computeActionHint();
    if (this._actionFeedback) {
      this._actionFeedback.timer -= dt;
      if (this._actionFeedback.timer <= 0) this._actionFeedback = null;
    }
    if (this._ringPulse > 0) this._ringPulse = Math.max(0, this._ringPulse - dt * 0.6);
    ...
    for (const r of SG.CONFIG.rings) if (healthPct >= r.unlockHealth) highestUnlocked = r.id;
    if (highestUnlocked > this._lastUnlockedRing) {
      const ring = SG.CONFIG.rings.find(r => r.id === highestUnlocked);
      this._lastUnlockedRing = highestUnlocked;
      this._ringUnlockBanner = { text: ring.name.toUpperCase() + ' UNLOCKED', timer: 3 };
      this._ringPulse = 1;
      this.audio.unlockRegion();
    }
    if (this._ringUnlockBanner) {
      this._ringUnlockBanner.timer -= dt;
      if (this._ringUnlockBanner.timer <= 0) this._ringUnlockBanner = null;
    }
    const self = this;
  ...
  render(dt) {
    ...
    this.renderer.updateShake(this.meteors.shakeAmount, this.ui.save.settings.screenShake);
    this.renderer.drawPlanet(this.world, this.world.restoreAreaPercent(), reducedMotion, this._ringPulse);
    if (this.actionHint && this.actionHint.type !== 'occupied') {
      this.renderer.drawActionHighlight(this.actionHint.patch, this.actionHint.type !== 'locked' && (this.actionHint.afford !== false), this.world);
    }
  ...
  _updateHudDom() {
    ...
    let toastText = '';
    if (this._actionFeedback) toastText = this._actionFeedback.text;
    else if (this._ringUnlockBanner) toastText = this._ringUnlockBanner.text;
    ...
    this.ui.updateHUD({
      ...
      actionHint: this.actionHint, toastText,
    });
  }

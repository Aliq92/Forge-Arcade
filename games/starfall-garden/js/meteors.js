...
SG.MeteorManager = class {
  ...
  _pickTarget(player) {
    ...
    const activeTargets = this.meteors.filter(m => m.state === 'warning' || m.state === 'falling');
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (SG.util.dist(x, y, player.x, player.y) < 70) continue;
      let tooClose = false;
      for (const m of activeTargets) {
        if (SG.util.dist(x, y, m.x, m.y) < minSeparation) { tooClose = true; break; }
      }

...
SG.Renderer = class {
  ...
  // -------- Planet & terrain --------
  drawPlanet(world, healthPct, reducedMotion, ringPulse) {
    const ctx = this.ctx;
    ...
    // brief bright pulse when a new ring unlocks
    if (ringPulse > 0) {
      ctx.save();
      ctx.globalAlpha = reducedMotion ? ringPulse * 0.4 : ringPulse;
      ctx.beginPath(); ctx.arc(c.x, c.y, R * (1 + (1 - ringPulse) * 0.08), 0, SG.util.TAU);
      ctx.lineWidth = 3 + ringPulse * 5;
      ctx.strokeStyle = SG.COLORS.healthyCyan;
      ctx.shadowColor = SG.COLORS.healthyCyan;
      ctx.shadowBlur = 24 * ringPulse;
      ctx.stroke();
      ctx.restore();
    }
  }

  // Soft pulsing outline over a patch the player can currently act on (restore / plant).
  drawActionHighlight(patch, ok, world) {
    const ctx = this.ctx;
    ...
    ctx.save();
    ctx.strokeStyle = ok ? SG.util.rgba(SG.COLORS.healthyCyan, 0.5 + 0.4 * pulse) : SG.util.rgba('#ff8080', 0.5 + 0.3 * pulse);
    ctx.lineWidth = 2.2;
    ctx.setLineDash([5, 4]);
    const r = half * (1.25 + 0.1 * pulse);
    ctx.beginPath();
    ctx.roundRect ? ctx.roundRect(s.x - r, s.y - r, r * 2, r * 2, half * 0.4) : ctx.rect(s.x - r, s.y - r, r * 2, r * 2);
    ctx.stroke();
    ctx.restore();
  }
  ...
  drawPlayer(player, reducedMotion) {
    ...
    // dash cooldown: small ring that fills back up as dash becomes ready again
    if (player.dashCooldownTimer > 0) {
      const dashCdMax = SG.CONFIG.player.dashCooldown * player.dashCooldownMul;
      const frac = SG.util.clamp(1 - player.dashCooldownTimer / dashCdMax, 0, 1);
      const start = -Math.PI / 2;
      ctx.strokeStyle = SG.util.rgba(SG.COLORS.healthyCyan, 0.55);
      ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(s.x, s.y, R * 1.55, start, start + SG.util.TAU * frac); ctx.stroke();
      ctx.strokeStyle = SG.util.rgba('#ffffff', 0.12);
      ctx.beginPath(); ctx.arc(s.x, s.y, R * 1.55, 0, SG.util.TAU); ctx.stroke();
    }

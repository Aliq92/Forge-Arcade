...
import {
  ...
  saveLastSeed, loadLastSeed, saveSystemSnapshot, loadSystemSnapshotResult,
} from './config.js';
import { attractors, getAttractor, removeAttractor, setMass, duplicateAttractor, createAttractor, clearAttractors, setNextAttractorId } from './attractors.js';
...
import { CHALLENGES, INTENTIONS, intentionState, getGardenProgress, challengeState, startChallenge, stopChallenge, updateChallenge } from './challenges.js';
import * as Sim from './simulation.js';
import { focusOnSelected, getCreativeToolVariants } from './tools.js?v=cosmic-gardener-8-fix3';
import { buildGardenHudState, describeSelectedBody } from './garden-hud.js';

let camera = null;
let canvas = null;
let setGardenStudioOpen = () => {};
...
function wireGardenStudio() {
  ...
  const seedTools = new Set(['star', 'planet', 'moon', 'influence', 'pulse']);
...
function wireGardenHud() {
  $('garden-intention').addEventListener('click', () => {
    showToast(`${$('garden-intention-label').textContent} · ${$('garden-intention-progress').style.width || '0%'}`);
  });
  $('btn-card-focus').addEventListener('click', () => focusOnSelected(camera));
  $('btn-card-follow').addEventListener('click', () => $('btn-cam-follow').click());
  $('btn-card-cradle').addEventListener('click', () => {
    if (state.selectedAttractorId == null) { showToast('Select a body to cradle first.'); return; }
    selectTool('move');
...
function refreshGardenHud() {
  ...
  $('selected-body-state').textContent = `${view.stage} · ${view.orbit}`;
  $('selected-body-warmth').textContent = view.warmth;
  $('selected-body-hint').textContent = state.cradleArmed
    ? 'Cradle armed — press and hold this body, then drag it to place it.'
    : view.cradled
    ? 'Cradled — release gently to restore its motion.'
    : 'Focus, follow, or hold and drag to cradle this body.';
}

let creativeVariantKey = '';
...
function wireSaveLoad() {
  $('btn-save-system').addEventListener('click', () => {
    const snapshot = {
      attractors: attractors.map(a => ({
        id: a.id, type: a.type, x: a.x, y: a.y, vx: a.vx, vy: a.vy, mass: a.mass,
        radius: a.radius, fixed: a.fixed, color: a.color, showTrail: a.showTrail, name: a.name,
        gardenStage: a.gardenStage, stageAge: a.stageAge, stableFor: a.stableFor,
        dominantStarId: a.dominantStarId, warmth: a.warmth, orbitHealth: a.orbitHealth,
        appearanceSeed: a.appearanceSeed, ringStrength: a.ringStrength, lastTransform: a.lastTransform,
      })),
  ...
  $('btn-load-system').addEventListener('click', () => {
    ...
    if (result.status === 'missing') { showToast('No saved garden found'); return; }
    if (result.status === 'invalid') { showToast('Saved garden could not be read'); return; }
    ...
    for (const a of snap.attractors || []) {
      createAttractor(a.type, a.x, a.y, {
        id: a.id, mass: a.mass, radius: a.radius, vx: a.vx, vy: a.vy, fixed: a.fixed, color: a.color,

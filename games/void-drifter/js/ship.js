...
class Player {
  constructor() {
    ...
    this.lastHitBy = null; // last hazard type that damaged the ship, used for the game-over death cause
  ...
  update(dt, input, settings, fuelMult = 1) {
    ...
    if (effectiveThrust && this.fuel > 0) {
      ...
      this.vx += fx * power * dt;
      this.vy += fy * power * dt;
      this.fuel = clamp(this.fuel - 6.2 * dt * (this.boosting ? 1.6 : 1) * fuelMult, 0, this.maxFuel);
    } else if (input.reverse && this.fuel > 0) {
      this.reversing = true;
      this.vx -= fx * this.reverseThrustPower * dt;
      this.vy -= fy * this.reverseThrustPower * dt;
      this.fuel = clamp(this.fuel - 4.2 * dt * fuelMult, 0, this.maxFuel);
    }
    ...
    // lightweight recent-motion trail so drift direction reads clearly without cluttering the screen
    this._trailTimer += dt;
    if (this._trailTimer > 0.035) {
      this._trailTimer = 0;
  ...
  draw(ctx, camera, w, h, time) {
    ...
    // recent-motion trail, drawn in world space before the ship-local transform below
    if (this.trail.length > 1) {
      ctx.save();
      ctx.lineCap = 'round';
      const pts = this.trail.concat([{ x: this.x, y: this.y }]);
      for (let i = 1; i < pts.length; i++) {
        const a = (i / pts.length) * 0.22;
        const p0 = pts[i - 1], p1 = pts[i];
        ctx.strokeStyle = `rgba(100,224,255,${a.toFixed(3)})`;
        ctx.lineWidth = 1 + (i / pts.length) * 1.5;
        ctx.beginPath();
        ctx.moveTo(p0.x - camera.x + w / 2, p0.y - camera.y + h / 2);
        ctx.lineTo(p1.x - camera.x + w / 2, p1.y - camera.y + h / 2);
        ctx.stroke();
      }
      ctx.restore();
    }

...
class EventManager {
  ...
  _trigger(player, world, onToast) {
    ...
    if (roll < 0.24) this._salvageSignal(player, world, onToast);
      ...
      // a leak on top of an already-near-empty tank can be an unwinnable starvation streak;
      // give the player a calm break instead when fuel is critical
      if (player.fuel / player.maxFuel < 0.25) this._calmPocket(onToast);
      else this._fuelLeak(onToast);
    }

...
class Camera {
  ...
  update(dt, targetX, targetY, targetVx, targetVy, shakeEnabled) {
    ...
    // restrained zoom-out at high speed (boost, mostly) so incoming hazards stay visible sooner
    const speed = Math.hypot(targetVx, targetVy);
    const zoomTarget = speed > 420 ? clamp(1 - (speed - 420) / 900, 0.88, 1) : 1;
    this.zoom = lerp(this.zoom, zoomTarget, 1 - Math.exp(-3 * dt));

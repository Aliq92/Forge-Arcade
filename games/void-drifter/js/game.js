...
class Game {
  constructor() {
    ...
    this.updateTitleBest(this.mode);
  ...
  startRun(mode) {
    ...
    this.stats = {
      distancePx: 0,
      maxDistancePx: 0,
      bestKm: SaveData.getBestDistanceKm(this.mode),
      sectorsSurvived: 0,
  ...
  updateTitleBest(mode) {
    this.ui.titleBest.textContent = SaveData.getBestDistanceKm(mode).toFixed(2) + ' km';
  }
  ...
  _endRun(title) {
    ...
    SaveData.setBestDistanceKm(pxToKm(this.stats.maxDistancePx), this.mode);
    this.stats.bestKm = SaveData.getBestDistanceKm(this.mode);
  ...
  stationLeave() {
    ...
    this.state = 'playing';
    this.ui.hideAllScreens();
    // brief grace period so undocking never drops the player straight into a hazard
    if (this.player) this.player.invulnTime = Math.max(this.player.invulnTime, 1.2);
  }
  ...
  update(dt) {
    ...
    if (this.player && this.state !== 'title') {
      ...
      this.ui.drawMinimap(this.player, this.world, getSectorInfo(pxToKm(this.stats.distancePx)), this._insideIonCloud, this.eventManager ? this.eventManager.beacons : [], this.settings.reducedMotion);
    }
  }
  ...
  _updatePlaying(dt) {
    ...
    player.update(dt, inputState, this.settings, this.eventManager.fuelConsumptionMult);
    ...
    // wide enough that the collision-risk warning (below) always has data to look at
    const queryRadius = Math.max(player.magnetRadius, 380, player.scannerRange * 0.5);
    ...
    for (const hz of nearby.hazards) {
      if (hz.type === 'ioncloud') {
        ...
        // swept segment check (matches the asteroid check above) so fast-moving fragments can't tunnel through the ship on a slow frame
        const cp = closestPointOnSegment(player.prevX, player.prevY, player.x, player.y, hz.x, hz.y);
        if (dist2(cp.x, cp.y, hz.x, hz.y) <= (player.radius + hz.radius) * (player.radius + hz.radius)) {
          const relSpeed = Math.hypot(player.vx - hz.vx, player.vy - hz.vy);
          const dmg = (hz.type === 'solar' ? 16 : 8) + relSpeed * 0.045;
          player.lastHitBy = hz.type;
    ...
    if (sectorInfo.name !== this.stats.currentSectorName) {
      ...
      this.ui.setSector(sectorInfo.name, sectorInfo.desc);
    }
    ...
    if (this._bestSaveAccum > 1) {
      this._bestSaveAccum = 0;
      SaveData.setBestDistanceKm(kmNow, this.mode);
      this.stats.bestKm = Math.max(this.stats.bestKm, kmNow);
    }
    ...
    if (player.destroyed) this._endRun(this._deathCauseTitle());
  }

  _deathCauseTitle() {
    const causes = { asteroid: 'ASTEROID COLLISION', mine: 'MINE IMPACT', solar: 'SOLAR FRAGMENT', debris: 'DEBRIS IMPACT' };
    return causes[this.player.lastHitBy] || 'VOID LOST';
  }
